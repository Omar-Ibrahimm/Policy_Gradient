"""
PPO Experiment Runner
Runs baseline + hyperparameter variants for PPO algorithm.
"""
import argparse
import json
import time
...
from common.trainer import train_ppo, save_model_and_config
...
def load_config(config_path="configs/ppo.json"):
    """Load PPO configuration from JSON file."""
...
def main():
    ...
    if args.project is None:
        args.project = f"rl-PPO-{args.env}"
    
    print(f"\n{'#'*60}")
    print(f"# PPO Experiment Suite")
    ...
    if len(all_results) > 0:
        comparison_dir = f"plots/{args.env.lower()}/ppo"
