"""
SAC Experiment Runner
Runs baseline + hyperparameter variants for SAC algorithm.
"""
import argparse
import json
import time
...
from common.trainer import train_sac, save_model_and_config
...
def load_config(config_path="configs/sac.json"):
    """Load SAC configuration from JSON file."""
...
def main():
    parser = argparse.ArgumentParser(description='Run SAC experiments')
    parser.add_argument('--env', type=str, required=True,
                       help='Environment ID (e.g., Pendulum-v1)')
    ...
    if args.project is None:
        args.project = f"rl-SAC-{args.env}"
    
    print(f"\n{'#'*60}")
    print(f"# SAC Experiment Suite")
    ...
    if len(all_results) > 0:
        comparison_dir = f"plots/{args.env.lower()}/sac"
